...
class contactManager {
    ...
    async updateContact() {
        ...
        const index = this.contactDetails.findIndex((contact) => contact.id === contactId);
        if (index === -1) {
    ...
    async deleteContact() {
        if (this.contactDetails.length <= 0) {
            ...
            const contactId = parseInt(idPrompt.id);
            const contactIndex = this.contactDetails.findIndex((contact) => contact.id === contactId);
            if (contactIndex === -1) {
                console.log(chalk.red("Invalid ID. Please enter a valid ID."));
            }
            else {
                const index = this.contactDetails.findIndex((contact) => contact.id === parseInt(idPrompt.id));
